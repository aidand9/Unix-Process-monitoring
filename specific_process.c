...
void specificProc(int pid, int per_proc_flag, int systemwide_flag, int vnode_flag, int composite_flag, int threshold){  // This function is called if the user passes a pid as an command line argument
   ...
   DIR* proc_dir = opendir("/proc");
    ...
    DIR* temp = opendir(pid_path);

    if(temp == NULL){
        printf("Unable to access process, exiting program.\n");                   
        exit(1);
    } 
    char path[500];
    if(per_proc_flag == 1){
            ...
            if (pid_uid == uid) {
                ...
                while ((fd_entry = readdir(fd_dir)) != NULL) {
                    if (strcmp(fd_entry->d_name,".") != 0 && strcmp(fd_entry->d_name,"..") != 0) {
                        ...
                        if(strcmp(proc_entry->d_name, s_pid) == 0){
